...
extension Row {
    ...
    fileprivate struct User: Codable, QueryParameter, Equatable {
        ...
        let createdAtDateComponents: DateComponents?
        ...
        private enum CodingKeys: String, CodingKey {
            ...
            case createdAtDateComponents = "created_at_datecomponents"
...
final class QueryTests: XCTestCase, QueryTestType {
    ...
    func createTestTable() throws {
        ...
        let query = """
            ...
            `created_at_datecomponents` datetime DEFAULT NULL,
    ...
    func testInsertRowCodable() throws {
        ...
        let dateComponents = DateComponents(year: 2012, month: 3, day: 4, hour: 5, minute: 6, second: 7, nanosecond: 890)
        
        let userNil = User(id: .noID, name: name, age: age, createdAt: someDate, createdAtDateComponents: dateComponents, nameOptional: nil, ageOptional: nil, createdAtOptional: nil, done: false, doneOptional: nil, userType: .user)
        ...
        let userFill = User(id: .ID(UserID(134)), name: name, age: age, createdAt: someDate, createdAtDateComponents: dateComponents, nameOptional: "fuga", ageOptional: 50, createdAtOptional: anotherDate, done: true, doneOptional: false, userType: .admin)
        ...
        let rows:[User] = try pool.execute { conn in
            try conn.query("SELECT id,name,age,created_at,created_at_datecomponents,name_Optional,age_Optional,created_at_Optional,done,done_Optional,user_type FROM ?? ORDER BY id ASC", [constants.tableName])
        }
    ...
    func testTransaction() throws {
        ...
        let user = Row.User(id: .noID, name: "name", age: 11, createdAt: someDate, createdAtDateComponents: nil, nameOptional: nil, ageOptional: nil, createdAtOptional: nil, done: false, doneOptional: nil, userType: .user)
    ...
    func testEmojiInserting() throws {
        ...
        let now = Date()
        let user = User(id: .noID, name: "日本語123🍣🍺あいう", age: 123, createdAt: now, createdAtDateComponents: nil, nameOptional: nil, ageOptional: nil, createdAtOptional: nil, done: false, doneOptional: nil, userType: .user)
        let status: QueryStatus = try pool.execute { conn in
            try conn.query("INSERT INTO ?? SET ? ", [constants.tableName, user])
        }
